...
### Sprint 1: 핵심 트래커 안정화 (완료/진행 중)
- [x] **Task 1.1**: YOLO11 Pose 모델 연동.
- [x] **Task 1.2**: Pose Similarity 기반 ID 매칭 로직 구현 (`tracker.py`).
- [x] **Task 1.3**: ID 떨림 방지 (Stabilization) 및 색상 구분 시각화.
- [ ] **Task 1.4**: 트래킹 로그 JSON 저장 기능 검증 (테스트 필요).

### Sprint 2: 서버 및 연동 기초 (금일 목표)
- [ ] **Task 2.1**: 트래커 로직을 API 서버(FastAPI) 래핑 검토 (또는 로컬 데모로 대체).
...
### 📅 오늘 제출 마일스톤
1.  **14:00**: 설계 문서(본 문서) 및 IA/Flow 확정.
2.  **16:00**: `tracker.py` 최적화 (파라미터 튜닝) 및 로그 저장 확인.
3.  **17:00**: 결과 영상 렌더링 및 문서 패키징.
4.  **18:00**: 제출.
...
### 7.1 테스트 레벨
- **단위 테스트 (Unit)**: `get_pose_similarity` 함수가 동일 인물에 대해 1.5 이하, 다른 인물에 대해 2.0 이상의 거리를 반환하는지 검증.
- **통합 테스트 (Integration)**: 영상을 `process_frame`에 넣었을 때 ID가 끊기지 않고 5프레임 이상 `CONFIRMED` 상태를 유지하는지 확인.

### 7.2 수용 기준 (Acceptance Criteria)
1.  **ID 유지성**: 화면 내에서 사람이 걷는 동안(약 5초간) ID가 변경되지 않아야 한다. (Success Rate > 80%)
